...
function binary_search(inputValue, low, high) {
    ...
    function animateAllToRed() {
        items.forEach((item, index) => {
            animateColor(index, 'red', 0); // Animate all divs to red immediately
        });
    }
    ...
    while (low <= high) {
        ...
        setTimeout(() => {
            items[low].style.backgroundColor = 'purple';
            items[middle].style.backgroundColor = 'cyan';
            items[high].style.backgroundColor = 'brown';
        }, 1000);

        if (array[middle] === inputValue) {
            // If key found, animate and display message
            animateColor(middle, 'green', 1000); // Found key becomes green
            setTimeout(() => {
            ...
            animateColor(low, 'purple', 1000); // Low index becomes purple
            low = middle + 1;
        } else {
            // Animate the updated high index
            animateColor(high, 'brown', 1000); // High index becomes brown
            high = middle - 1;
        }
    }
